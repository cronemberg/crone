...
export let viewMoreText: KeyTextField = "Dive In";
...
<ul on:mouseleave={onMouseLeave} class="grid border-b border-b-slate-100">
    ...
    class="content-list-item" use:onItemEnter={index}>
...
<!-- Hover element -->
 <div class="hover-reveal pointer-events-none absolute left-0 top-0 -z-10 h-[320px] w-[220px] rounded-lg bg-cover bg-center opacity-0 transition-[background] duration-300"
 style={currentIndex == undefined ? '' : `background-image: url(${contentImages[currentIndex]})`}></div>